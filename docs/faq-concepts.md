...
5. Livox 是固态或混合固态雷达么？
   ...
   - 不是，是宏观机械扫描当中的双飞镜惯性扫描，而且是一种非均布的渐进收敛与多帧积分扫描，2枚机械飞镜具备2个正交扫描分量（分别高频加减速振动），是MEMS微振镜方案的宏观具象，而且还内置风扇-_-||~
   - 本身等价帧率（约1hz）与点云时间密度(约10-24万点每秒)相对偏低，后续算法需要重建，无法复用VLP多年累积的长期训练数据，包括历史积累的数据集及训练模型，都无法平滑迁移。
