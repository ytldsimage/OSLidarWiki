...
1. 什么是数字激光雷达，MBF与传统方案的区别是？
   ...
   基于此，无论多少线，OS雷达内永远只有收发合计2枚芯片，发射端用VCSEL将多颗发射单元及后端处理线路放在一颗SOC上，接收端自研Spads ASIC将全部像素及后端处理电路全部聚集在一颗芯片上，随着线数增加，成本、结构、体积、器件复杂度、设备可靠性没有很大变化，一路标定全部标定，一次标定终生标定，像素元芯片内固化锁定，即可定时又可定角发射，VFOV方向是结构化并发无畸变数据，HFOV方向仍然是结构化数据，代表着更高的集成度、可靠性与数据前标定质量。
...
8. 什么叫时空锁定机制，有什么价值？
   ...
   数据结构示意图：![image54](faq-concepts.assets/image54.gif)
