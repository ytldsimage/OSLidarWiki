...
>
> ![image-20200917210554905](faq.assets/image-20200917210554905.png)
...
5. Livox 是固态或混合固态雷达么？
   ...
   - 由于实际上其实属于单像素（或《6像素）时间轮询串发，所以任何方位角上（我们只算纵向就好）都是不同的时间戳，都需要做各速度分量的位移时间补偿；且测绘类应用与航向非正交部分是扫描分量无意义，进一步削弱等价帧率与有效点云密度。
...
11. 如果将该雷达用于海岸线监控，或防波堤监控，长期在盐雾海水蒸汽的氛围下，OS雷达的可靠性如何？

    ASTM B117-19 salt spray test , per the General Motors quality/reliability guidelines. 1,000 hour duration, 24 hour cycle.
...
15. Ubuntu下寻找全部雷达IP的命令是？

    ```sh
    avahi-browse -lr _roger._tcp
    ```

16. Ubuntu 下正确的ROS驱动安装编译方式是？

    安装ROS驱动步骤：
    - 进入ouster_example -> ouster_viz文件夹，打开README.md, 按照里面的Build Dependencies步骤完成对dependencies的安装；
    2. 安装好 ouster_viz后进入ouster_example -> ouster_client文件夹，打开README.md, 按照里面的步骤build好client;
...
18. 请问如何理解极坐标到笛卡尔坐标转换过程中，azimuth角度当中有2个构成分量？
...
19. IMU是硬连接还是软连接，输出原始观测量还是解算值，fps是否可调？
...
21. 工作温度指的是环境温度，还是雷达自身温度？有无进一步拓宽温度区间的计划？
...
22. 如果移除底板与顶部散热片，在密闭室内过热，会导致雷达烧坏么？发现OS雷达长期工作会有些烫手，这正常么？
...
36. 目前128线 2048*10hz为例，每秒钟的总点云数目是多少，计入IMU数据后，数据量是多少，用bag或pcap格式？
...
40. 默认线材是多长的？是否符合低煙無鹵（Low smoke zero halogen）簡稱LSZH 要求？

    默认配5m ibox straight light cable，需要特别长度，不同角度connector，以及pigtail需要特别提出。

    线材符合LSZH要求。

41. 脉冲dtof，脉冲itof的区别是什么？
