...
16. 如果将该雷达用于海岸线监控，或防波堤监控，长期在盐雾海水蒸汽的氛围下，OS雷达的可靠性如何？

    ASTM B117-19 salt spray test , per the General Motors quality/reliability guidelines. 1,000 hour duration, 24 hour cycle.S

17. OS的雷达，pixel distance也会随着时间或振动发生变动么，也需要不超过2000hr必须重新校准么？

    不需要，像素元芯片内空间锁定固化，设计寿命内不发生变动

18. 是否有可能采用低帧率高解析度（4096@5fps，8192@2.5fps）的选项提高雷达的性能，适应低动态环境？
...
22. 对传统AGV与激光叉车而言，升级到Ouster方案的效益体现在哪里？
...
26. 目前128线 2048*10hz为例，每秒钟的总点云数目是多少，计入IMU数据后，数据量是多少，用bag或pcap格式？
...
28. 如仅仅处理雷达报文的解析与转换，需要的算力是多少，如果针对比如OS2-128 选配NVIDIA处理板卡?
    ...
    ![telegram-cloud-photo-size-1-5157165441209182365-y](faq-applications.assets/telegram-cloud-photo-size-1-5157165441209182365-y.jpg)

29. 目前雷达输入电压是多少，支持宽电压输入么？
...
30. 目前是多次回波方案么？
    ...
    目前选择的是最强回波方案，我们计划在2021Q2加入多次回波; 由于我们除了环视雷达之外，还有环视纹理相机的存在，后者其实可以补偿目前阶段的一些涉及多回波相关的应用需求。

31. 目前相机是额外一套cmos相机么，最高像素是多少，是彩色相机么？
...
35. ambient,signal, intensity以及reflectivity数据有什么区别?
...
36. 我们政府/公民对隐私比较敏感，不希望雷达当中有环境光相机的存在，OS可以提供移除ambient数据的隐私模式么？

    是的，可以的。我们有专门推出移除ambient camera的纯粹雷达点云与强度/反射率信息的方案。

37. 如何在不同距离上估算雷达的落点？在XXm处多大物体切割多少个点正常么？
...
40. 工作温度指的是环境温度，还是雷达自身温度？有无进一步拓宽温度区间的计划？
...
41. 如果移除底板与顶部散热片，在密闭室内过热，会导致雷达烧坏么？发现OS雷达长期工作会有些烫手，这正常么？
...
42. 目前ROS驱动，是否已经将lidar坐标系转为sensor坐标系，是否已经将IMU的位姿信息转换为sensor坐标系？
...
43. 采用ROS VIZ来可视化点云数据，且以intensity着色的时候，发现强度闪烁的事情是正常的么？
