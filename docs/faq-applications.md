...
1. 雷达的标定文件从哪里获取？
   ...
   - ROS：工作目录默认路径在`${ROS_HOME}` 也就是`$HOME/.ROS`，所以如果`metadata`设定的是相对路径，会放在该目录内，否则，请指定绝对路径：
     ...
     >
     > 此处特别注意，不仅指定路径，还要输入`XYZ.json`的文件名！！
...
12. SYNC_PULSE_IN：This input channel is protected by an optoisolator which will draw 10mA at full turn on，这个光隔离器的作用是？我理解这个脚位是外部时间同步信号，是一个脉冲信号，为什么会需要光隔离？
...
25. 如何在驱动层故意丢弃数据，模拟低线数的采样情况？
    ...
    - 或者直接修改源代码，在ros.cpp中修改这一段代码（这一段代码就是用来计算os_cloud_node/points里每个点的信息并进行排布的，代码中for循环里的那个u就是通道channel的序号）：
      ...
      ![wecom-temp-d497f98b56408ad545ae33830558a25d](faq-applications.assets/wecom-temp-d497f98b56408ad545ae33830558a25d.png)
...
26. 是否可以将roslaunch后的许多参数都省掉，或设置成另外一种默认参数？

    - 首先，记得设置雷达为静态IP，且本机IP手动指定为同一局域网IP及子网掩码；

    - 其次，可以通过GUI浏览器界面，或TCP/API将各种参数比如发射模式，帧率，静态IP等设置为所需的IP，用浏览器界面的时候先点apply，再点save；或者命令行的时候写入`save_config_params`.

    - 此时重启后，会按照上述参数自动运行，但如希望roslaunch命令行后面直接去除全部参数，以robot_upstart插件启动roslaunch，以`rosrun package_name executable_file_name`形态，而不采用`metadata` 和`hostname`这2个非可选参数（类似rviz），可以更改launch文档当中的default值即可。

      ![wecom-temp-9baca7180ce36a99326a4f5132637057](faq-applications.assets/wecom-temp-9baca7180ce36a99326a4f5132637057-3858601.jpg)
...
30. 对传统AGV与激光叉车而言，升级到Ouster方案的效益体现在哪里？
...
31. 基于Velodyne的旋转机械雷达，转一圈实际上点云的时间戳是周期性的变换，这样当车在运动的过程中，扫到的一面墙很可能就不是平的了，需要结合车的运动信息进行补偿，才能得到真实的点云，OS雷达也是这样么？OS雷达当中的时间戳是发射时间戳，还是接收时间戳？
...
34. 目前128线 2048*10hz为例，每秒钟的总点云数目是多少，计入IMU数据后，数据量是多少，用bag或pcap格式？
...
40. 目前相机是额外一套cmos相机么，最高像素是多少，是彩色相机么？
    ...
    ![image-20211212133620336](faq-applications.assets/image-20211212133620336.png)

41. 不同激光雷达反映在点云的数据结构上有什么区别？
...
44. ambient,signal, intensity以及reflectivity数据有什么区别?
...
45. 我们政府/公民对隐私比较敏感，不希望雷达当中有环境光相机的存在，OS可以提供移除ambient数据的隐私模式么？

    是的，可以的。我们有专门推出移除ambient camera的纯粹雷达点云与强度/反射率信息的方案。

46. 如何在不同距离上估算雷达的落点？在XXm处多大物体切割多少个点正常么？
...
50. 如果移除底板与顶部散热片，在密闭室内过热，会导致雷达烧坏么？发现OS雷达长期工作会有些烫手，这正常么？
...
51. 目前ROS驱动，是否已经将lidar坐标系转为sensor坐标系，是否已经将IMU的位姿信息转换为sensor坐标系？
...
52. 采用ROS VIZ来可视化点云数据，且以intensity着色的时候，发现强度闪烁的事情是正常的么？
